...
func Write(conf config.Config, plugin plugins.Plugin, version toolversions.Version, executablePath string) error {
	...
	if reservedShimNames[shimName] {
		// Also remove any shim that may already exist under this reserved
		// name, e.g. one generated by a version of asdf that predates this
		// guard. `asdf reshim` (no args) already clears the whole shims
		// directory via RemoveAll before regenerating, but `asdf reshim
		// <tool> <version>` calls Write directly without doing so, so
		// leaving a stale shims/asdf in place here would let the original
		// recursion bug persist for users on that path until they happen to
		// run a full reshim.
		shimPath := Path(conf, shimName)
